...
/**
 * One row of the explorer tree — a folder or a diagram.
 *
 * `memo`'d, and that is the whole reason it is a separate file: the tree is flattened upstream
 * (`lib/diagrams/tree.ts`) precisely so typing in the search box rebuilds the row *list* without
 * re-rendering every row in it.
 *
 * **Every callback prop is stable and takes the row as an argument.** This is the first half of
 * making the `memo` real: handlers written inline in the parent's `.map()` are a fresh function
 * identity on every parent render, so the shallow compare fails for every row, every time, and the
 * component is memo'd in name only. The parent `useCallback`s these once and the row passes itself
 * back in.
 *
 * **`sameRow` is the second half, and without it the first buys nothing.** `flattenTree` mints a
 * fresh wrapper object for every row on every rebuild. A default shallow compare would see a
 * hundred new `row` props and re-render a hundred rows because one diagram's title moved. What
 * actually decides whether a row looks different is the identity of the folder or diagram *inside*
 * the wrapper, which `diagramsStore` preserves for every row it did not touch — so the comparator
 * reaches through it.
 */
function sameRow(a: Row, b: Row): boolean {
  if (a.kind !== b.kind || a.id !== b.id || a.depth !== b.depth) return false;
  if (a.kind === "folder" && b.kind === "folder") {
    return a.folder === b.folder && a.diagramCount === b.diagramCount;
  }
  if (a.kind === "diagram" && b.kind === "diagram") return a.diagram === b.diagram;
  return false;
}

export const DiagramTreeRow = memo(
  function DiagramTreeRow({
     ...
     *
     * `into` washes the whole row — it is being filed into this folder. `before`/`after` draw a
     * line along the edge the item would be inserted at, which is the only honest way to show a
     * *position*: a highlighted row would say "in here" when what is about to happen is "next to
     * here".
     */
    dropEdge: DropEdge;
    /** This row owns the tree's single tab stop. */
    focused: boolean;
    /** *This* row is being dragged, so it dims rather than highlighting under the pointer. */
    dragging: boolean;
    /**
     * What an untitled diagram reads as.
     *
     * Passed in rather than looked up here: `useT` subscribes to `languageStore`, and a
     * subscription per row is a hundred subscriptions re-checked on every store write in the app.
     * The parent holds the one subscription and hands down a string.
     */
     ...
     * is live, so it is the one that reads `event.currentTarget`.
     */
    ...
    return (
      <div
        ...
        // which. A tree where every row takes a tab stop is a tree nobody can tab past — this is
        // the pattern the ARIA tree spec asks for, and the reason the key handling lives on the
        // container rather than here.
        tabIndex={focused ? 0 : -1}
        data-diagram-row={row.id}
        onClick={() => (isFolder ? onToggle(row) : onSelect(row))}
        ...
        // *within* a row, which is what the gesture "a little higher than that" is made of.
      ...
      >
        {/* The insertion line. Absolutely positioned and `pointer-events-none` so it can straddle
            the row's edge without ever being what the pointer is over — a target that moves out
            from under the cursor as it appears is a target you cannot aim at. */}
        {(dropEdge === "before" || dropEdge === "after") && (
          <span
            aria-hidden
            className={`pointer-events-none absolute inset-x-0 h-0.5 rounded-full bg-[var(--cf-accent-fill)] ${
        ...
        {isFolder ? (
          ...
          // The chevron's width, kept as empty space, so diagram titles line up with folder names
        ...
        <span className={`min-w-0 flex-1 truncate ${isFolder ? "font-medium" : ""}`}>
          {isFolder ? (
            row.folder.name
          ) : row.diagram.title ? (
            row.diagram.title
          ) : (
            // Italic and muted so it reads as the absence of a title rather than as a diagram
  ...
  // The wrapper object is new on every tree rebuild; the folder or diagram inside it is not. See
  // the component's doc comment — this is what makes the memo hold across an unrelated write.
  (previous, next) =>
