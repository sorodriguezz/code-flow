...
export function EmptyState({
  ...
  return (
    ...
    <div className="flex h-full w-full flex-col items-center justify-center gap-1.5 p-8 text-center">
      ...
      <Icon size={24} strokeWidth={1.75} className="mb-1.5 shrink-0 text-[var(--cf-text-faint)]" />
      <p className="text-[14px] font-medium text-[var(--cf-text)] [text-wrap:balance]">{title}</p>
      {subtitle && (
        <p className="max-w-[300px] text-[12px] leading-relaxed text-[var(--cf-text-muted)] [text-wrap:balance]">
          {subtitle}
        </p>
      )}
