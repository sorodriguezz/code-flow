 ...
 *
 * A plain `title` gets the same box now, too: `TitleTooltips` draws every one in the window with
 * this component's bubble, delay and warm-up. So a bare `title` is fine for a truncated cell or a
 * one-line hint; reach for `<Tooltip>` when the label needs structure — a description line, a key
 * cap, a colour dot — or a preferred side.
 */
 ...
 *  Shared with `TitleTooltips` too, through the two functions below, so running the pointer from a
 *  control with a `<Tooltip>` to one with a plain `title` is still one gesture. */
let warmUntil = 0;

/** Whether a tooltip closed a moment ago — the next one opens with no wait. */
export function tooltipIsWarm(): boolean {
  return Date.now() < warmUntil;
}

/** Call when a tooltip that was actually shown closes: starts the warm window. */
export function tooltipClosed(): void {
...
export function Tooltip({
  ...
  return (
    <>
      <span
        ref={wrapRef}
        // Marks the controls that already have a label of ours, so `TitleTooltips` knows not to
        // draw a second one over a stray `title` inside.
        data-cf-tooltip=""
...
/**
 * The box itself — measured, placed beside `anchor`, portalled — without any of the hovering that
 * decides when it shows. `Tooltip` drives it for the controls that declare a label, and
 * `TitleTooltips` for every element that only carries a `title`, so the two look and move exactly
 * alike.
 */
export function TooltipBubble({
  ...
  return createPortal(
    <div
      ...
      style={
        {
          left: pos?.left ?? 0,
          top: pos?.top ?? 0,
          // Rendered before it is placed, so it can be measured — hidden rather than transparent,
          // because a transparent box still animates, and it would play its entrance at the wrong
          // coordinates.
      ...
      // The shadow lives in `.cf-tip` rather than a Tailwind class — it is two layers, and one of
      // them is mixed per theme. See `index.css`.
      className="cf-tip pointer-events-none fixed z-[70] max-w-[17rem] rounded-lg border border-[var(--cf-border)] bg-[var(--cf-surface-raised)] px-2.5 py-1.5"
    >
      ...
      {description && (
        <p className="mt-1 whitespace-pre-line break-words text-[11px] leading-snug text-[var(--cf-text-muted)]">
          {description}
        </p>
      )}
    </div>,
