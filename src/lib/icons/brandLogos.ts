...
export const BRAND_LOGOS: Record<string, BrandLogo> = {
   ...
   * `logos:openai-icon`.
   *
   * Codex is OpenAI's CLI, so it wears OpenAI's own mark rather than one invented for the CLI.
   * There used to be an `openai` row beside this carrying the identical path, for the
   * OpenAI-compatible engine; that engine is gone and this is now the only copy.
   */
  codex: {
    width: 256,
    height: 260,
