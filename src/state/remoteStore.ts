 ...
 *
 * It *was* in `App`, in one effect that switched `apiStore`, `dbStore` and this store together — and an effect in `App` runs
 * in the **main window only**. A satellite holds its own workspace and picks it from its own title
 * bar (see `SatelliteApp`), so a detached Remote window changed workspace and then went on
 * showing — and writing to — the one it was opened in. Nothing announced that: the tree simply did
 * not change, which reads as "the hosts are global" rather than as a bug.
 *
...
useWorkspaceStore.subscribe((state, previous) => {
  ...
  void useRemoteStore.getState().setWorkspace(state.activeWorkspaceId);
});
